...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    @Query(
            ...
            countQuery = "SELECT COUNT(c) FROM Comment c WHERE c.member = :member"
    )
    Page<Comment> findAllByMemberOrderByCreatedAtDesc(@Param("member") Member member, Pageable pageable);

    // 게시글 삭제 시 해당 게시글의 댓글 일괄 삭제
    void deleteAllByPost(Post post);

    // 게시글의 전체 댓글 한 번에 조회 (root + children) — fetch join으로 N+1 방지
