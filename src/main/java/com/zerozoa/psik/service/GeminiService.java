...
public class GeminiService {
    ...
    private static final String ANALYSIS_PROMPT = """
        이 이미지가 사람의 얼굴 사진인지 먼저 확인
        얼굴이 잘 탐지되지 않거나, 1명 이상이라면 반드시 아래 JSON만 반환해:
        {"error": "FACE_NOT_DETECTED"}
        
        얼굴이 잘 탐지됐다면 피부 전문가 관점에서 분석 후 아래 JSON만 반환해:
        {
          "acne_score": 여드름 점수 0~100(여드름이 많으면 0, 적으면 100에 가깝게),
          "wrinkle_score": 주름 점수 0~100(팔자, 눈가, 피부 탄력을 종합해서 주름이 많으면 0, 없으면 100에 가깝게),
